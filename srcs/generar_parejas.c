...
int leer_participantes(FILE *f, Pareja participantes[], int *num_part)
{
	...
	while (fgets(linea, sizeof(linea), f) != NULL) {
		if (linea[0] == '\n' || linea[0] == '#') {     // Ignorar líneas vacías o comentarios (que empiezan con #)
			continue;
		}
		if (*num_part >= MAX_PAREJAS) {		// Código mejorable! se puede concatenar para un num indefinido
			printf("Error: Demasiados participantes. :(\n");
			return 1;
		}
		linea[strcspn(linea, "\n")] = '\0';	// Esta funcion recorta el salto de línea al final del string
...
int validar_archivo_participantes(FILE *f)
{
	...
	while (fgets(linea, sizeof(linea), f) != NULL) {	// Verificar que haya al menos 2 participantes
	...
	rewind(f);	// Reiniciar el puntero del archivo al inicio del mismo
	return 0;
}
