...
int generar_enfrentamientos(Pareja participantes[], int num_part, Partido enfrentamientos[]) {
    ...
    for (int i = 0; i < num_part; i += 2) {
        if (i + 1 < num_part) {
            // Asignar los índices correctos de las parejas
            enfrentamientos[num_enfrentamientos].pareja1 = i;
            enfrentamientos[num_enfrentamientos].pareja2 = i + 1;
            num_enfrentamientos++;
        }
    }
...
void guardar_enfrentamientos(Partido enfrentamientos[], int num_enfrentamientos, Pareja participantes[]) {
    FILE *f = fopen("enfrentamientos.txt", "w");
    if (f == NULL) {
    ...
    fclose(f);
    printf("Enfrentamientos guardados en 'enfrentamientos.txt'.\n");
}
